-- Zero Hub - Blox Fruits Script
-- Dev: nexora.xyz

if getgenv().ZeroHub then 
...
if executor then
    if
        ...
        string.find(executor, "Cryptic") or
		string.find(executor, "Volt")
    then
        print("Okie")
    else
        game.Players.LocalPlayer:Kick("Please Use Delta If You Are On Phone And If You Are On PC Than Use YuB - X Or A Paid Executor Like Volt!")
    end
end
...
local T1UIColor = {
    ...
    ["Click Effect Color"] = Color3.fromRGB(200, 200, 200),      -- White/Gray 
    ["Setting Icon Color"] = Color3.fromRGB(200, 200, 200),
    ["Logo Image"] = "rbxassetid://140318122086207",            -- New logo
    ["Notification Image"] = "rbxassetid://115125408886765",
    ["Search Icon Color"] = Color3.fromRGB(180, 180, 180),
    ["Search Icon Highlight Color"] = Color3.fromRGB(220, 220, 220),
    ...
    ["Title Text Color"] = Color3.fromRGB(220, 220, 220),        -- Monochrome
    ...
    ["Dropdown Selected Color"] = Color3.fromRGB(200, 200, 200),
    ["Dropdown Selected Check Color"] = Color3.fromRGB(200, 200, 200),
    ["Textbox Highlight Color"] = Color3.fromRGB(200, 200, 200),
    ["Box Highlight Color"] = Color3.fromRGB(200, 200, 200),
...
Library_Function.Gui.Name = 'Zero Hub GUI'
...
btnHide.Image = "rbxassetid://140318122086207"   -- Zero Hub logo on hide button
...
btnStroke.Color = Color3.fromRGB(200, 200, 200)   -- White accent
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "Zero Hub"
	...
	maingui.ImageColor3 = Color3.fromRGB(200, 200, 200)   -- White border
	...
	uistr.Thickness = 1;
	uistr.Color = Color3.fromRGB(200, 200, 200);   -- White accent
	...
	TopStroke.BackgroundColor3 = Color3.fromRGB(200, 200, 200)   -- White
	...
	TextLabelMain.Text = "<font color=\"rgb(200,200,200)\">Zero Hub</font> <font color=\"rgb(255,255,255)\">- Blox Fruit</font>"
	...
	pageControlStroke.Color = Color3.fromRGB(200, 200, 200)
	...
	function Main_Function:AddTab(PageName)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			sectionStroke.Color = Color3.fromRGB(200, 200, 200)
			...
			function sectionFunction:AddToggle(idk,Setting)
				...
				check.BackgroundColor3 = Color3.fromRGB(200, 200, 200)   -- White toggle check
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            TextColor_1.Parent = RowBG_1
             ...
             ClickArea_1.BackgroundColor3 = Color3.fromRGB(200, 200, 200)   -- White button
	...
	UIS.InputBegan:Connect(function(input, gpe)
		...
		for _, key in ipairs(_movementKeys) do
			if input.KeyCode == key then
				if not _G.ZeroStartFarm and not _G.SailBoats and not _G.ZeroAutoTyrant then
					shouldTween = false;
					_G.StopTween = false;
				end;
				break;
...
InfoTab:AddButton({
	Title = "Join On Zero Hub Community!",
	Callback = function()
		setclipboard("https://discord.gg/YVKbRBprdh")  -- discord invite
		Library:Notify({
			Title = "Zero Hub",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto kill The Saw boss for Saw Sword",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto kill nearby players for Usoap Hat",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto buy Bones randomly to summon bosses",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto kill Greybeard for Bisento - Sea 1",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto kill Chief Warden for Warden Sword",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto Citizen Quest to unlock Ken V2",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto kill Don Swan for Swan Coat - Sea 2 Swan Room",
	...
	while task.wait(0.5) do
		pcall(function()
			...
			if not hasFrag then
				local mob = GetConnectionEnemies("Darkbeard");
				if mob then
					FHNotify("Skull Guitar", "Killing Darkbeard...", 3);
...
MainTab:AddDropdown({
	...
	Desc = "Level = by quest/level | Bone = Haunted Castle | Cake Prince | Tyrant Of The Skies | Nearest = attack nearest NPCs",
...
local function _vStage5_Raid()
	...
	if _G.VolcanicCollectEgg and _G.FullyVolcanicActive then
		Library:Notify({Title="Volcanic Kaitun", Content="Collecting Dragon Egg...", Icon="egg", Duration=4});
	...
	if _G.VolcanicCollectBone and _G.FullyVolcanicActive then
		Library:Notify({Title="Volcanic Kaitun", Content="Collecting Dino Bones...", Icon="box", Duration=4});
		local bt=0;
...
_volcanicMainLoop = function()
	while _G.FullyVolcanicActive do
		...
		if not workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
			...
			Library:Notify({Title="Volcanic Kaitun", Content="[4/9] Going to Tiki Outpost and sailing to Sea 6+...", Icon="anchor", Duration=5});
			_vStopTween();
			shouldTween = false; _G.StopTween = true;
			local boatName = _G.VolcanicSelectedBoat or "Guardian";
			local hrp = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart");
			...
			Library:Notify({Title="Volcanic Kaitun", Content="[5/9] Sailing... waiting for Prehistoric Island to spawn...", Icon="ship", Duration=6});
			_vSailToIsland(700);
		end;
		if not _G.FullyVolcanicActive then break; end;

		if workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
			Library:Notify({Title="Volcanic Kaitun", Content="[6/9] Prehistoric Island detected! Heading there via Tween...", Icon="map-pin", Duration=4});
			_G._volcanicPhase = "tweening";
			_vStage4_GoToIsland();
			Library:Notify({Title="Volcanic Kaitun", Content="[7/9] On the island! Activating the volcano raid...", Icon="zap", Duration=3});
		end;
		if not _G.FullyVolcanicActive then break; end;

		_G._volcanicPhase = "solving";
		Library:Notify({Title="Volcanic Kaitun", Content="[8/9] RAID ACTIVE! Patching Holes + Killing Golems...", Icon="shield", Duration=5});
		_vStage5_Raid();
		if not _G.FullyVolcanicActive then break; end;

		Library:Notify({Title="Volcanic Kaitun", Content="[9/9] Fully completed the volcanic event solo! Waiting for reset…", Icon="trophy", Duration=8});
		...
		if _G.VolcanicAutoReset then
			...
			if _G.FullyVolcanicActive and _G.VolcanicAutoReset then
				Library:Notify({Title="Volcanic Kaitun", Content="Resetting character for new cycle...", Icon="refresh-cw", Duration=3});
				pcall(function()
	...
	_G._volcanicPhase = "idle";
	Library:Notify({Title="Volcanic Kaitun", Content="Auto Fully Volcanic Disabled.", Icon="x", Duration=4});
end;
...
MultiFarmTab:AddDropdown({
	...
	Desc    = "Boat used to navigate to Prehistoric Island",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Scrap Metal → Blaze Ember (kill + tree) → Craft Magnet → Search → Raid → Collect → Reset.",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Wait 10 seconds and reset after the raid.",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Collect Dragon Egg apos raid.",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Collecting Bones after a raid.",
...
MultiFarmTab:AddButton({
	...
	Desc  = "// Remove all lava from Prehistoric Island now",
	Callback = function()
		pcall(function()
			for _, v in pairs(workspace:GetDescendants()) do
		...
		Library:Notify({Title="Volcanic", Content="Lava removed!", Icon="check", Duration=3});
	end
});
...
task.spawn(function()
	...
	plr.CharacterAdded:Connect(function(char)
		...
		task.wait(4);
		if not _G.FullyVolcanicActive then return; end;
		Library:Notify({Title="Volcanic Kaitun", Content="// Character resurfaced - resuming cycle..", Icon="refresh-cw", Duration=4});
		task.spawn(_volcanicMainLoop);
	end);
end);
...
MultiFarmTab:AddParagraph({
	...
	Desc = "Enter the Dungeon, defeat all NPCs on every floor (including Kitsune on F10 and Gas on F15), perform a Skip Hub, and repeat. This works only in the Dungeon World. (Place ID: 73902483975735)."
});
...
AutoFarmChestTweenToggle = OthersTab:AddToggle({
	...
	Desc = "Precise tween to each chest. Confirms collection via Beli before moving to the next.",
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Zero Hub", Content = "Special item found! Auto Chest stopped.", Icon = "bell", Duration = 6});
				return;
			end;
			local chests = {};
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Zero Hub", Content = "Special item found!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Zero Hub", Content = "Chest Hop: limit reached, switching server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
...
AutoFarmChestInstantToggle = OthersTab:AddToggle({
	...
	Desc = "Instant and precise teleport to each chest. Waits for collection confirmation before the next.",
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Zero Hub", Content = "Special item found! Chest Bypass stopped.", Icon = "bell", Duration = 6});
				return;
			end;
			local chests = {};
			for _, v in pairs(workspace.ChestModels:GetChildren()) do
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Zero Hub", Content = "Chest Hop: switching server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
OthersTab:AddToggle({
	Title = "Chest Hop",
	Desc = "Switches server after collecting X chests (set the amount below)",
	Value = false,
	Callback = function(state)
...
OthersTab:AddDropdown({
	...
	Desc = "How many chests to collect before server hopping",
...
task.spawn(function()
	...
	for _, v in pairs(Tabs.OthersTab._elements or {}) do
		if type(v) == "table" and v.Title == "Chests Collected" then para = v; break; end;
	end;
	while true do
		task.wait(1);
...
local function ShowResetConfirm()
	pcall(function()
		...
		msg.Text = "Are you sure you want to reset all settings?";
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			Library:Notify({Title = "Zero Hub", Content = "Settings reset! Re-execute the script.", Icon = "bell", Duration = 6});
		end);
...
SettingsTab:AddToggle({
	...
	Desc = "Enabled by default. Uses movement obfuscation, interval randomization, and human-like action simulation to reduce detection by the Blox Fruits AC.",
	Value = true,
	Callback = function(state)
...
SettingsTab:AddSlider({
	...
	Desc = "Maximum distance (in studs) to attack nearest NPCs. Island default = 150",
	Min = 20,
	Max = 600,
...
ResetSettingsButton = SettingsTab:AddButton({
	...
	Desc = "Deletes all saved settings (asks for confirmation)",
	Callback = function()
		ShowResetConfirm();
	end
});
...
SettingsTab:AddSlider({
	Title = "Jump Power %",
	Desc = "100% = normal jump (50). 500% = 250. Maximum: 1000%",
	Min = 100,
	Max = 1000,
...
SettingsTab:AddToggle({
	Title = "Fast M1 Fruit",
	Desc = "Spams fruit clicks very fast when Weapon = Fruit and farming anything. In Sea Beast with special fruits (Pain, T-Rex, Kitsune, Control, Dragon) uses only M1 without skills.",
...
SettingsTab:AddToggle({
	Title = "FPS Boost",
	Desc = "Removes textures and visual effects to improve FPS and reduce lag.",
	Value = false,
	Callback = function(state)
...
FarmingHopTab:AddToggle({
	Title = "Auto Yama CDK",
	Desc = "Auto completes all Yama quest steps for CDK. Sea 3.",
	Value = false,
	Callback = function(state)
...
FarmingHopTab:AddToggle({
	...
	Desc = "Equips legendary Hakis (Pink/White/Red) and activates the castle Pads. Switches every 7s.",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Automatically brings the God Chalice to the tomb to spawn Rip Indra.",
	...
	while task.wait(0.5) do
		pcall(function()
			...
			if _G.Settings.Raid["Auto Raid"] then
				if raidActive then
					RaidStatusParagraph:SetDesc(" Raid in progress! Killing enemies...");
				else
					if RaidGetBP("Special Microchip") then
						RaidStatusParagraph:SetDesc(" Microchip ready. Starting raid...");
					else
						RaidStatusParagraph:SetDesc(" Buying Microchip...");
					end;
				end;
			else
				RaidStatusParagraph:SetDesc(" Offline");
			end;
		end);
	end;
end);
...
ChooseChipRaidDropdown = RaidTab:AddDropdown({
	Title = "Choose Chip (Fruit for Microchip)",
	Desc = "Select which fruit to trade for the Microchip",
...
AutoRaidToggle = RaidTab:AddToggle({
	...
	Desc = "Starts the raid when the chip is in the bag and kills all enemies on the islands. Does not buy the chip.",
...
AutoBuyChipToggle = RaidTab:AddToggle({
	Title = " Auto Buy Chip",
	Desc = "Swaps a fruit already in the bag for the Microchip. Does not move fruits from storage. Buys only 1 chip at a time.",
	Value = false,
	Callback = function(state)
...
AutoAwakeningToggle = RaidTab:AddToggle({
	Title = "Auto Awaken",
	Desc = "Automatically awakens the fruit during the raid",
...
AutoUnstoreDevilFruitToggle = RaidTab:AddToggle({
	...
	Desc = "Removes fruits from inventory below the defined price (default: under 1,000,000). Use together with Auto Buy Chip.",
...
if not _IS_DUNGEON then
	...
	DungeonTab:AddToggle({
		...
		Desc = "Goes to the ring NPC in the lobby and fuses rings automatically.",
	...
	DungeonTab:AddToggle({
		...
		Desc = "Spins rings automatically at the lobby NPC.",
	...
	DungeonTab:AddToggle({
		...
		Desc = "Teleports to the yellow floor at the Dungeon entrance and keeps trying to start the match.",
	...
	DungeonTab:AddToggle({
		...
		Desc = "Attacks all nearby NPCs on each floor, destroys Kitsune Shrines (Floor 10), destroys gas leaks (Floor 15), and advances automatically.",
	...
	DungeonTab:AddToggle({
		...
		Desc = "Enables automatic buff selection when the card screen appears in the dungeon.",
	...
	DungeonTab:AddDropdown({
		...
		Desc = "Choose which buffs should be selected automatically (multi-selection)",
		Values = _KNOWN_BUFFS,
	...
	DungeonTab:AddToggle({
		...
		Desc = "When the dungeon ends, automatically presses to return to the lobby.",
...
local PvPPlayerDropdown = CombatTab:AddDropdown({
	Title = "Select Player",
	Desc = "Selects the target player for PvP",
...
CombatTab:AddButton({
	...
	Desc = "Refreshes the player list on the server",
	Callback = function()
		RefreshPvPTargets();
		pcall(function()
...
CombatTab:AddButton({
	...
	Desc = "Teleports to the selected player's position",
	Callback = function()
		pcall(function()
...
CombatTab:AddToggle({
	...
	Desc = "Follows and stays above the selected player constantly",
	Value = false,
	Callback = function(v)
		_PvPAutoTP = v;
	end
...
CombatTab:AddToggle({
	...
	Desc = "Keeps PvP activated automatically",
	Value = false,
	Callback = function(v)
		if v then
			task.spawn(function()
...
CombatTab:AddToggle({
	...
	Desc = "Automatically kills the selected player",
	Value = false,
	Callback = function(v)
		_PvPAutoKill = v;
	end
...
CombatTab:AddToggle({
	Title = "AimLock NPC",
	Desc = "Locks camera onto the nearest NPC automatically",
	Value = false,
	Callback = function(v)
		_G.AimLockNPC = v;
	end
...
CombatTab:AddToggle({
	...
	Desc = "Locks camera onto the selected player automatically",
	Value = false,
	Callback = function(v)
		_G.AimLockPlayer = v;
	end
...
CombatTab:AddToggle({
	...
	Desc = "Redirects skill projectiles/hits to the target without needing to look at it",
	Value = false,
	Callback = function(v)
		_G.SilentAimSkill = v;
	end
...
CombatTab:AddToggle({
	...
	Desc = "Redirects hits to the nearest NPC without looking at it",
	Value = false,
	Callback = function(v)
		_G.SilentAimNPC = v;
	end
...
CombatTab:AddToggle({
	...
	Desc = "Redirects hits to the selected player without looking at it",
	Value = false,
	Callback = function(v)
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Zero Hub", Content = "Style not found: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Zero Hub", Content = "Purchase sent: " .. styleName, Icon = "check", Duration = 4});
end;

ShopTab:AddButton({
	Title = " Go Buy Selected Fight Style",
	Desc = "Tweens to the NPC, completes the dialog, and buys the selected style",
	Callback = function()
...
ShopTab:AddToggle({
	...
	Desc = "Goes to each NPC in order and buys all fight styles",
	Value = false,
	Callback = function(state)
...
SelectedTeleportIslandDropdown = TeleportTab:AddDropdown({
	Title = "Choose Island",
	Desc = "Dynamic Eclipse list - reads the actual map",
...
AutoTeleportToIslandToggle = TeleportTab:AddToggle({
	...
	Desc = "Moves smoothly to the selected island and stops when it arrives. Player can move normally.",
	Value = false,
	Callback = function(state)
...
TeleportTab:AddToggle({
	...
	Desc = "Bypass teleports (with reset) to the selected island. Repeats until arrival. Player can jump/move.",
	Value = false,
	Callback = function(state)
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Zero Hub", Content = "Arrived at: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
...
AutoTweenToIslandToggle = TeleportTab:AddToggle({
	Title = "Tween To Island",
	Desc = "Moves smoothly to the selected island",
	Value = false,
	Callback = function(state)
...
SelectedNpcTeleport = TeleportTab:AddDropdown({
	Title = "Choose Npc",
	Desc = "Dynamic Eclipse list - real server NPCs",
...
TeleportToNpcToggle = TeleportTab:AddToggle({
	...
	Desc = "Instant teleport directly to the NPC",
	Value = false,
	Callback = function(state)
...
TweenToNpcToggle = TeleportTab:AddToggle({
	Title = "Tween To Npc",
	Desc = "Moves smoothly to the selected NPC",
	Value = false,
	Callback = function(state)
		_G.TweenNPC = state;
		if not state then StopTween(false); end;
...
DragonDojoTab:AddParagraph({
	...
	Desc = "Automatically completes belt quests (White -> Black)"
});
...
DragonDojoTab:AddToggle({
	Title = "Auto Dojo Trainer",
	Desc = "Starts and completes belt quests: White, Yellow, Green, Purple, Red, Black",
	Value = false,
	Callback = function(I)
		_G_Dojoo = I;
		_G.Dojoo = I;
...
ChooseZoneDropdown = SeaEventTab:AddDropdown({
	...
	Desc = "Danger level for navigation (Sea 3 only)",
...
SeaEventTab:AddToggle({
	...
	Desc = "Applies custom boat speed in real time",
	Value = false,
	Callback = function(v)
		_G.SpeedBoat = v;
	end
	...
	pcall(function()
		while task.wait(0.2) do
			...
			if game.Workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island") then
				KitsuneStatusSeaStackParagraph:SetDesc("Kitsune Island Spawned");
			else
				KitsuneStatusSeaStackParagraph:SetDesc("Kitsune Island Not Spawn");
			end;
			if game.Workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension") then
				FrozenStatusSeaStackParagraph:SetDesc("Frozen Dimension Spawned");
			else
			...
			if game.Workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
				PrehistoricStatusSeaStackParagraph:SetDesc("Prehistoric Island Spawned");
			else
...
SeaStackTab:AddToggle({
	Title = "Auto Find Prehistoric Island",
	Desc = "Auto navigates to Sea 6+ to find the island.",
	Value = false,
	Callback = function(state)
		_G.Prehis_Find = state;
	end
...
SeaStackTab:AddToggle({
	...
	Desc = "Remove lava + Kill Golems + Fill holes until the event ends.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "When it finds the Leviathan island, goes to the NPC and activates it automatically.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "Detects live Leviathan tails and goes to them, spamming all skills (Z X C V F).",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "When Leviathan dies, gets the Beast Hunter boat, centers it at the location, and shoots the heart.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddDropdown({
	Title = "Auto Drive Boat - Destination Island",
	Desc = "Select the destination island for Auto Drive Boat",
...
SeaStackTab:AddToggle({
	...
	Desc = "Mounts the nearest Beast Hunter boat and tweens to the selected island.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "Buys a boat at Tiki, goes to sea Lv 4, patrols until Mirage spawns. If the boat breaks, resets and restarts.",
...
SeaStackTab:AddToggle({
	...
	Desc = "Goes to Mirage's highest point, stares at the moon. When the moon glows at night, teleports to the gear.",
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Zero Hub", Content = "Quest complete!", Icon = "bell", Duration = 6});
						end);
...
AutoStoreFruitToggle = FruitTab:AddToggle({
	...
	Desc = "Checks inventory and stores eligible fruits quickly.",
...
TeleportToFruitToggle = FruitTab:AddToggle({
	...
	Desc = "BAN RISK - Instantly teleports to the fruit. Use with caution.",
...
TweenToFruitToggle = FruitTab:AddToggle({
	...
	Desc = "Moves smoothly to the fruit and stops farming when it appears.",
...
task.spawn(function()
	while true do
		...
		pcall(function()
			...
			if not found then _raidBossStatusParagraph:SetDesc("No Raid Boss active"); end;
		end);
	end;
end);
task.spawn(function()
...
HoldAndSkillTab:AddParagraph({
	Title = "How it works",
	Desc = "Select below which keys/skills will be used in ALL farms and hub functions requiring skills. The combinations Z X C Melee, Z X C V F Fruit, Z X Sword and Z X Gun define fast groups."
});
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill Z",
	Desc = "Enables using the Z key in farms",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill X",
	Desc = "Enables using the X key in farms",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill C",
	Desc = "Enables using the C key in farms",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill V",
	Desc = "Enables using the V key in farms",
	Value = false,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill F",
	Desc = "Enables using the F key in farms",
	Value = false,
	Callback = function(state)
...
task.spawn(function()
	...
	if S.Multi and S.Multi["Auto Fully Volcanic"] then
		...
		task.spawn(_volcanicMainLoop);
		Library:Notify({Title="Volcanic Kaitun", Content="Auto Fully Volcanic reactivated!", Icon="zap", Duration=5});
	end;

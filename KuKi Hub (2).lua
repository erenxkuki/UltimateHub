...
local T1UIColor = {
	...
	["Title Text Color"] = Color3.fromRGB(255, 80, 80),         -- Light Red
	...
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(120, 0, 0),        -- Dark Red
	["Section Text Color"] = Color3.fromRGB(200, 200, 200),
	["Section Underline Color"] = Color3.fromRGB(180, 0, 0),    -- Red
	["Toggle Border Color"] = Color3.fromRGB(180, 0, 0),        -- Red
	["Toggle Checked Color"] = Color3.fromRGB(255, 100, 100),   -- Light Red
	["Toggle Desc Color"] = Color3.fromRGB(150, 150, 150),
	["Button Color"] = Color3.fromRGB(180, 0, 0),               -- Red
	["Label Color"] = Color3.fromRGB(0, 0, 0),
	["Dropdown Icon Color"] = Color3.fromRGB(200, 200, 200),
	["Dropdown Selected Color"] = Color3.fromRGB(120, 0, 0),    -- Dark Red
	["Dropdown Selected Check Color"] = Color3.fromRGB(80, 0, 0), -- Dark Red
	["Textbox Highlight Color"] = Color3.fromRGB(180, 0, 0),    -- Red
	["Box Highlight Color"] = Color3.fromRGB(180, 0, 0),        -- Red
	["Slider Line Color"] = Color3.fromRGB(180, 0, 0),          -- Red
	["Slider Highlight Color"] = Color3.fromRGB(120, 0, 0),     -- Dark Red
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto Kill The Saw boss And Get Saw Sword",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Automatically buy Bones at random to summon bosses.",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto-kill Darkbeard — Hops servers if not found.",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto kill Chief Warden And Auto Get Warden Sword",
...
FarmingHopTab:AddToggle({
	...
	Desc = "“Someone got clapped by an Elite Boss, and the boss dropped loot.”",
...
MultiFarmTab:AddParagraph({
	...
	Desc = "Enter the Dungeon, defeat all NPCs on each floor (including Kitsune on F10 and Gas on F15), perform a Skip Hub, and repeat. This works only in Dungeon World (Place ID: 73902483975735)."
});

local _AutoFullyDungeonToggle = MultiFarmTab:AddToggle({
	Title = "Auto Fully Dungeon",
	Desc = "Toggles the full automatic Dungeon cycle on/off..",
...
task.spawn(function()
	...
	while true do
		...
		if game.PlaceId ~= _DUNGEON_PID then
			...
			Library:Notify({Title = "Auto Fully Dungeon", Content = "It's Not In Dungeon World! Place ID: " .. _DUNGEON_PID, Icon = "alert-triangle", Duration = 5});
			task.wait(10);
			continue;
		end;
		pcall(function()
...
CombatTab:AddToggle({
	...
	Desc = "Directs the skill's projectiles/hits toward the target without needing to look at it.",
	Value = false,
	Callback = function(v)
		_G.SilentAimSkill = v;
	end
...
CombatTab:AddToggle({
	...
	Desc = "Redirects hits to the selected player without looking at them.",
	Value = false,
	Callback = function(v)
...
AutoKillPlayerAfterTrialToggle = RaceTab:AddToggle({
	Title = "Auto Kill Player After Trials",
	Value = _G.Settings.Race["Auto Kill Player After Trials"],
	Callback = function(value)
		_G.Settings.Race["Auto Kill Player After Trials"] = value;
		(getgenv()).SaveSetting();
	end
});
spawn(function()
	while wait(0.2) do
		if _G.Settings.Race["Auto Kill Player After Trials"] then
						...
						if (v.Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 500 then
							if v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
								repeat
									...
									Attack();
								until not _G.Settings.Race["Auto Kill Player After Trials"] or (not v.Character) or v.Character.Humanoid.Health <= 0;
							end;
						end;
					end;
				end;
...
ChooseBoatDropdown = SeaEventTab:AddDropdown({
	Title = "Choose Boat",
	Desc = "Type of Boat to Buy and Use",
...
ChooseZoneDropdown = SeaEventTab:AddDropdown({
	...
	Desc = "Navigation Hazard Level (Sea State 3 only)",
...
SeaEventTab:AddToggle({
	...
	Desc = "Applies custom speed to the boat in real time.",
	Value = false,
	Callback = function(v)
		_G.SpeedBoat = v;
	end
...
SailSeaToggle = SeaEventTab:AddToggle({
	Title = "Sail Sea",
	Desc = "Buys a boat, equips it, and navigates automatically between zones, attacking sea events. Stops upon encountering enemies.",
	Value = false,
	Callback = function(state)
		_G.SailBoat = state;
		_G.SailBoats = state;
	...
	pcall(function()
		while wait(0.2) do
			...
			if game.Workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island") then
				...
				KitsuneStatusSeaStackParagraph:SetDesc("Kitsune Island Not Spawned");
			end;
			if game.Workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension") then
				FrozenStatusSeaStackParagraph:SetDesc("Frozen Dimension Spawned");
			else
				FrozenStatusSeaStackParagraph:SetDesc("Frozen Dimension Not Spawned");
			end;
...
SeaStackTab:AddToggle({
	...
	Desc = "When you find the Leviathan island, go up to the NPC and it will activate automatically.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "Detect the Leviathan's living tails and head towards them, spamming all your skills (Z X C V F).",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "When Leviathan dies, take the Beast Hunter boat, center it on the location, and shoot the heart.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddDropdown({
	...
	Desc = "Select the destination island for the Auto Drive Boat.",
...
SeaStackTab:AddToggle({
	...
	Desc = "Board the nearest Beast Hunter boat and travel via Tween to the selected island.",
	Value = false,
	Callback = function(state)
...
SeaStackTab:AddToggle({
	...
	Desc = "Buy a boat at Tiki, head out to Sea Level 4, and patrol until Mirage spawns. If the boat breaks, reset and start over.",
...
SeaStackTab:AddToggle({
	...
	Desc = "Go to the highest point of Mirage and stare fixedly at the moon. When the moon shines at night, teleport to the gear.",
...
AutoStoreFruitToggle = FruitTab:AddToggle({
	...
	Desc = "Checks inventory and quickly stores eligible fruits.",
